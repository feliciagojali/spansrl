...
def main():
    ...
    f1_train, f1_test, f2_train, f2_test, f3_train, f3_test, out_train, out_test = train_test_split(features_1, features_2, features_3, out,test_size=0.4,train_size=0.6)
    f1_test, f1_val, f2_test, f2_val, f3_test, f3_val, out_test,out_val = train_test_split(f1_train,f2_train,f3_train,out_train,test_size = 0.5,train_size =0.5)
    type = {
        "train": [f1_train, f2_train, f3_train, out_train],
        "test": [f1_test, f2_test,f3_test, out_test],
        "val": [f1_val, f2_val, f3_val, out_val]
    }
    filename = [config['features_1'],config['features_2'],config['features_3'], config['output']]
