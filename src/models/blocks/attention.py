...
class Attention(Layer):
    ...
    def call(self, input): # Shape input: (batch_size, max_tokens, emb)
        ...
        span_indices = tf.minimum(
          tf.expand_dims(tf.range(self.max_span_width), 0) + tf.expand_dims(self.start, 1),
          self.max_tokens - 1)
        # Shape span_emb: (batch_size, num_spans, max_span_width, emb)
        ...
        # Shape span_indices_mask = (num_spans, max_span_width)
        span_indices_mask = tf.sequence_mask(span_width, self.max_span_width, dtype=tf.float32)
        span_indices_log_mask = tf.math.log(span_indices_mask)

        # Shape span_head: (batch_size, num_spans, max_span_width, 1)
