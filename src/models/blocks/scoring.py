...
class Scoring(Layer):
    def __init__(self, config, output_size, name, **kwargs):
        ...
        self.dense = []
        self.dropout = []
        for i in range(config['unary_depth']):
            self.dense.append(Dense(config['unary_units'], activation='relu', name=name+str(i)))
...
class ComputeScoring(Layer):
    ...
    def call(self, inputs):
        ...
        # Shape pair_score: (batch_size, num_preds, num_args, emb)
        # Shape biaffine_score: (batch_size, num_preds, num_args, emb)
        ...
        arg_score = tf.tile(tf.expand_dims(arg_unary_score, 1), [1, num_preds, 1, self.num_labels])
        pred_score = tf.tile(tf.expand_dims(pred_unary_score, 2), [1, 1, num_args, self.num_labels])

        # Shape total_score: (batch_size, num_preds, num_args, num_labels)
        total_score = self.add([arg_score, pred_score, pair_score, biaffine_score])
        # Shape null_score: (batch_size, num_preds, num_args, 1)
        null_score = self.null_score([arg_unary_score, pred_unary_score])
        # Shape final_score: (batch_size, num_preds, num_args, num_labels+1)
