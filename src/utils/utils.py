...
def load_data(config, types, full):
     # Features loading
    ...
    if (types=='train' and not full):
            input = [features_1[1:], features_2[1:], features_3[1:]]
            out = np.load(dir + config['output'], mmap_mode='r')[1:]
    elif (types == 'val' and full):
        n = 1
